...
export class BaseCategoryDocumentResponse implements Serialize<object>, Deserialize<string> {
  ...
  toString(): string {
    return `BaseCategoryDocumentResponse [category=${this.getCategory?.getName ?? ''}, document=${
      this.getDocument?.getTitle ?? ''
    }]`;
  }
}
